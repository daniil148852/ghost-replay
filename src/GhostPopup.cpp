...
void GhostPopup::refreshList() {
    ...
    float totalHeight = std::max(cellHeight * m_records.size(), 200.0);
...
CCNode* GhostPopup::createRecordCell(const RecordFileInfo& info, int index) {
    ...
    // Количество кадров
    char framesBuf[32];
    snprintf(framesBuf, sizeof(framesBuf), "%d frames", info.meta.totalFrames);
...
void GhostPopup::onToggleRecord(CCObject* sender) {
    ...
    bool enabled = !toggle->isToggled(); // isToggled returns pre-toggle state
    
    auto& info = m_records[idx];
    FileManager::get()->setRecordEnabled(info.fullPath, enabled);
    info.enabled = enabled;
    
    // Если мы в уровне, обновляем призраков
    ...
    log::info("Record {} {} ({})", idx, enabled ? "enabled" : "disabled", info.filename);
}
...
void GhostPopup::onDeleteAll(CCObject* sender) {
    geode::createQuickPopup(
        "Delete All",
        fmt::format("Delete ALL recordings for this level?"),
        "Cancel", "Delete All",
        [this](auto*, bool btn2) {
            if (btn2) {
...
void GhostMainPopup::refreshList() {
    ...
    std::map<int, std::pair<std::string, int>> levelMap; // levelID -> (name, count)
    ...
    float totalHeight = std::max(cellHeight * levelMap.size(), 190.0);
...
void GhostMainPopup::onSelectLevel(CCObject* sender) {
    ...
    // Открываем попап для конкретного уровня
